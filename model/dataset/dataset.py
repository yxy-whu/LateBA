...
class BERTDataset(Dataset):
    def __init__(self, cfg_corpus, vocab, seq_len, encoding="utf-8", corpus_lines=None, on_memory=True):
        self.vocab = vocab
        ...
        # with open(cfg_corpus, "r", encoding=encoding) as f:
        #     if self.corpus_lines is None and not on_memory:
        #         for _ in tqdm.tqdm(f, desc="Loading Dataset", total=corpus_lines):
        #             self.corpus_lines += 1
        ...
        # load CFG sequences 
        if self.corpus_lines is None and not on_memory:
            for _ in tqdm.tqdm(cfg_corpus, desc="Loading Dataset", total=corpus_lines):
                self.corpus_lines += 1

        if on_memory:
            self.cfg_lines = [line
                    for line in tqdm.tqdm(cfg_corpus, desc="Loading Dataset", total=corpus_lines)]
    ...
    def __getitem__(self, item):
        ...
        #randomly generate sentence and label
        ...
        # add padding
        padding = [self.vocab.pad_index for _ in range(self.seq_len - len(cfg_bert_input))]
        cfg_bert_input.extend(padding), cfg_bert_label.extend(padding), cfg_segment_label.extend(padding) #, comp_label.extend(padding)

...
class BERTTrainer:
    ...
    def iteration(self, epoch, data_loader, train=True):
        ...
        for i, data in data_iter:
            ...
            cfg_next_sent_output, mask_lm_output= self.model.forward(data["cfg_bert_input"], data["cfg_segment_label"])
            ...
            # prediction accuracy
            correct = cfg_next_sent_output.argmax(dim=-1).eq(data["cfg_is_next"]).sum().item()
            avg_loss += loss.item()
            total_correct += correct
            total_element += data["cfg_is_next"].nelement()

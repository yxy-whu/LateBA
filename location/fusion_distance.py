...
def get_data_db(data_path):
    ...
    return func_to_sequence, sequence_order, len(os_name)
...
def learning_function_train(train_dataset, train_loader, model, use_gpu):
    ...
    for epoch in range(num_epoches):    
        ...
        for i, data in enumerate(train_loader, 1):
            ...
            #print(num_correct)
            # print(num_correct.item()/batch_size)
    ...
    Y.append(y)
    # fmt = [['b', '', '-'], ['g', '', '-'], ['r', '', '-'], ['y', '', '-']]
    # line_name = ['normal', '5-Func', '10-Func', '15-Func']
    fmt = [['b', '', '-']]
    line_name = ['normal']
        
    # print graph for acc
    normal = Graph(X, Y, [0, num_epoches], [0.0, 1.0], 'tmp_1')
    normal.x2y('Accracy', 'Epoch', 10, 10, 2, False, fmt, line_name)
...
def main():
    ...
    train_data_txt, train_func_index, type_num_train = get_data_db(data_path)

    # getting instruction sequence data and fusion data for test task
    test_data_txt, test_func_index, type_num_test = get_data_db(test_data_path)
    ...
    model_after = learning_function_train(train_dataset, train_loader, model, use_gpu)
